...
export function CustomerFormModal({ isOpen, onClose, onSuccess, customer }: CustomerFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Frontend TCKN validasyonu: sadece 11 haneli rakam kabul et
    ...
    if (!village.trim()) {
      setIsSubmitting(false);
      setError("Lütfen müşterinin köyünü girin.");
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm animate-in fade-in">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl animate-in zoom-in-95">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">
              Köy <span className="text-rose-500">*</span>
            </label>
            <input
              required
              type="text"
